...
class tlAdapter:
    def __init__(self, dut, port_names, protocol=TL_UL, block_size=64, debug=False):
        ...
        if self.a_datalen != self.d_datalen:
            raise Exception(
                "{} a_data and d_data must have same width".format(dut.name)
            )
    ...
    def set_src_msgs(self, src_msgs, src, msgs):
        assert src not in src_msgs.keys(), "{} already in src_msgs".format(src)
    ...
    def updateMem(self, memory, burst_data):
        for addr, value in burst_data:
            ...
            memory[addr] = (memory.get(get_addr, 0) & (~bit_mask)) | (
                get_data & bit_mask
            )
    ...
    def retrieveBlock(
        self, b_srcs, b_callback, callback, param, size, addr, mask, msg="ProbeBlock"
    ):
        if not b_srcs.empty():
            self.retrieveBlock_cb(
                msg, b_srcs, b_callback, callback, param, size, addr, mask
            )
        else:
            b_reserve = CallBack(
                self.retrieveBlock_cb,
    ...
    def retrieveBlock_cb(
        self, msg, b_srcs, b_callback, callback, param, size, addr, mask
    ):
        b_src = b_srcs.get()
        b_callback.set(b_src, callback)
        self.b_queue.push(
            msg, param=param, size=size, source=b_src, address=addr, mask=mask
        )
    ...
    def AccessAckData_cb(self, memory, burst_len, addr_aligned, size, source):
        d_msgs = self.get_d_messages(
            "AccessAckData", memory, burst_len, addr_aligned, size=size, source=source
        )

        self.d_queue.push_msgs(d_msgs)

    def ArithmeticAck_cb(
        self, operand1, memory, burst_len, addr_aligned, bit_mask, offset, size, source
    ):
        operand2 = (memory[addr_aligned] & bit_mask) >> offset
        result = (
            self._arithmetic_op(param, operand1, operand2, mask) << offset
        ) & self.a_ports.data_mask

        assert burst_len == 1, "ArithmeticAck_cb, burst_len should be 1"

        memory[addr_aligned] = (memory.get(addr_aligned, 0) & (~bit_mask)) | (
            result & bit_mask
        )
    ...
    def LogicalAck_cb(
        ...
        operand2 = (memory[addr_aligned] & bit_mask) >> offset
        result = (
            self._logical_op(param, operand1, operand2, mask) << offset
        ) & self.a_ports.data_mask
    ...
    def probe_blocks_rec(self, probe_addrs, b_srcs, b_callback):
        ...
        if probe_addrs:
            ...
            self.retrieveBlock(b_srcs, b_callback, callback, toN, size, addr, mask)

        else:
            self.drive = False

    def drive_input(self, memory):
        assert memory.__class__.__name__ == "dict", "tlAdapter.drive_input need dict"
        ...
        self.a_monitor = cocotb.fork(
            self.a_port_monitor(memory, block_perm, d_sinks, b_srcs, b_callback)
        )
        self.c_monitor = cocotb.fork(
            self.c_port_monitor(memory, block_perm, b_srcs, b_callback)
        )
        ...
        self.retriever = cocotb.fork(
            self.data_retriever(block_perm, b_srcs, b_callback)
        )
    ...
    def a_port_monitor(self, memory, block_perm, d_sinks, b_srcs, b_callback):
        ...
        while self.drive:
            if a_ports.fire():
                ...
                assert (
                    not ongoings or source in ongoings.keys()
                ...
                bit_mask = int(
                    "".join(
                        ["{:02x}".format(0xFF * int(i)) for i in ("{:0b}".format(mask))]
                    ),
                    16,
                )
                ...
                if opcode == GET:
                    ...
                    if block_perm[block_addr] != TIP:
                        ...
                        self.retrieveBlock(
                            b_srcs, b_callback, callback, toT, size, addr, mask
                        )
                ...
                if opcode == PUT_FULL_DATA:
                    ...
                    if block_perm[block_addr] != TIP:
                        if count == 0:
                            ...
                            self.retrieveBlock(
                                b_srcs, b_callback, callback, toN, size, addr, mask
                            )
                        ...
                        if count + 1 == burst_len:
                            self.d_queue.push(
                                "AccessAck", None, size=size, source=source
                            )
                            if count:
                                ongoings.pop(source)
                        else:
                ...
                " TL-UH "
                if opcode == ARITHMETIC_DATA and self.protocol >= TL_UH:
                    ...
                    if block_perm[block_addr] != TIP:
                        ...
                        result = (
                            self._arithmetic_op(param, operand1, operand2, mask)
                            << offset
                        ) & self.a_ports.data_mask  # TODO, check _arithmetic_op

                        memory[get_addr] = (memory[get_addr] & (~bit_mask)) | (
                            result & bit_mask
                        )
                        self.d_queue.push(
                            "AccessAckData",
                ...
                if opcode == LOGICAL_DATA and self.protocol >= TL_UH:
                    ...
                    if block_perm[block_addr] != TIP:
                        ...
                        operand2 = (memory[get_addr] & bit_mask) >> offset
                        result = (
                            self._logical_op(param, operand1, operand2) << offset
                ...
                if opcode == INTENT and self.protocol >= TL_UH:

                    self.d_queue.push("HintAck", None, size=size, source=source)

                " TL-C "
                if opcode == ACQUIRE_BLOCK and self.protocol == TL_C:
                    ...
                    if block_perm[block_addr] != TIP:
                        ...
                        self.retrieveBlock(
                            b_srcs, b_callback, callback, b_param, size, addr, mask
                        )

                    else:
                        callback_d = CallBack(
                            self.updatePerm, block_perm, block_addr, d_param
                        )
                        d_msgs = self.get_d_messages(
                            "GrantData",
                            memory,
                ...
                if opcode == ACQUIRE_PERM and self.protocol == TL_C:
    ...
    def host_interface(self, block_perm, b_srcs, b_callback):
        ...
        while self.drive:
            if (
                self.probe & self.probe_en
            ) and self.probe_addr not in self.ongoing_tlc.values():
                block_addr = self.probe_addr & self.block_mask
                mask = (1 << self.b_datalen) - 1
                size = int(math.log(self.block_size, 2))

                if block_addr in block_perm.keys():
                    assert (
                        block_addr in block_perm.keys()
