...
def Run(
    ...
    assert toplevel in ["RocketTile", "BoomTile"], "{} is not toplevel".format(toplevel)

    random.seed(time.time() * (proc_num + 1))

    (mutator, preprocessor, isaHost, rtlHost, checker) = setup(
        dut, toplevel, template, out, proc_num, debug, no_guide=no_guide
    )
    ...
    while True:
        ...
        if isa_input and rtl_input:
            ...
            if ret == SUCCESS:
                ...
                match = True
                debug_print(
                    "[DifuzzRTL] Memory access outside DRAM -- {}".format(iNum),
                    debug,
                    True,
                )
                if record:
                    save_mismatch(
                        out, proc_num, out + "/illegal", sim_input, data, iNum
                    )
                iNum += 1
            ...
            if not match or ret not in [SUCCESS, ILL_MEM]:
                ...
                if record:
                    save_mismatch(
                        out, proc_num, out + "/mismatch", sim_input, data, mNum
                    )
