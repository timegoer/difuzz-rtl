...
def isa_timeout(out, stop, proc_num):
    ...
    shutil.copy(
        out + "/.input_{}.elf".format(proc_num), out + "/isa_timeout/timeout.elf"
    )
...
def setup(
    dut, toplevel, template, out, proc_num, debug, minimizing=False, no_guide=False
):
