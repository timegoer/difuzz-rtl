...
class sigChecker:
    ...
    def read_symbols(self, symbols):
        ...
        xreg_idxes = [
            (symbols["reg_x{}_output".format(i)] - symbol_start) // 8 for i in range(32)
        ]
        freg_idxes = [
            (symbols["reg_f{}_output".format(i)] - symbol_start) // 8 for i in range(32)
        ]
        csr_idxes = {}
    ...
    def read_sig(
        self, sigfile, xreg_idxes, freg_idxes, csr_idxes, data_symbols, data_idx_start
    ):
        ...
        for i in range(6):
            ...
            for j in range(section_size // 16):
                if data_idx_start + j < lines.__len__():
    ...
    def check_intr(self, symbols):
        ...
        (rtl_xreg_vals, rtl_freg_vals, rtl_csr_vals, rtl_data_vals) = self.read_sig(
            self.rtl_sigfile,
            xreg_idxes,
            freg_idxes,
            csr_idxes,
        ...
        assert (scause >> 63) & (
            mcause >> 63
        ) & 1 == 0, "Only one of Supervisor or Machine interrupt can be asserted"
        ...
        self.debug_print(
            "[DifuzzRTL] {} interrupt handled -- epc {:016x}".format(
                prv[intr_prv], epc
            ),
            intr_prv != NONE,
        )
    ...
    def check(self, symbols):
        ...
        (isa_xreg_vals, isa_freg_vals, isa_csr_vals, isa_data_vals) = self.read_sig(
            self.isa_sigfile,
            xreg_idxes,
            freg_idxes,
            csr_idxes,
        ...
        for i, val in enumerate(zip(isa_xreg_vals, rtl_xreg_vals)):
            ...
            if not match:
                xreg_match = False
            self.debug_print(
                "(x{:02} |{:>5}) [ISA] {:016x} || [RTL] {:016x}".format(
                    i, xreg_names[i], val[0], val[1]
                ),
                not match,
            )
        ...
        for i, val in enumerate(zip(isa_freg_vals, rtl_freg_vals)):
            ...
            if not match:
                xreg_match = False
            self.debug_print(
                "(f{:02} |{:>5}) [ISA] {:016x} || [RTL] {:016x}".format(
                    i, freg_names[i], val[0], val[1]
                ),
                not match,
            )
        ...
        for csr_name in csr_names:
            ...
            if not match:
                csr_match = False
            self.debug_print(
                "({:>10}) [ISA] {:016x} || [RTL] {:016x}".format(
                    csr_name, isa_val, rtl_val
                ),
                not match,
            )
        ...
        for i in range(6):  # TODO, max_sections = 6
            ...
            if isa_data != rtl_data:
                ...
                for j, words in enumerate(zip(isa_data, rtl_data)):
                    ...
                    self.debug_print(
                        "({:016x}) [ISA] {:016x} || [RTL] {:016x}".format(
                            addr, isa_word, rtl_word
                        ),
                        not match,
                    )
