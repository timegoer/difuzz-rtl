...
def wait_check(proc_pid_arr: list, manager: procManager):
    ...
    if state != proc_state.NORMAL:
        print(
            "[DifuzzRTL] Child {} is in abnormal state {}!".format(
                idx, proc_state.tpe[state]
            )
        )
        for pid in proc_pid_arr:
            if pid != end_pid:
...
if not multicore:
    ...
    for i in range(multicore):
        ...
        if not os.path.isfile(out + "/coverage/cov_log_{}_{}.txt".format(date, i)):
            save_file(
                out + "/coverage/cov_log_{}_{}.txt".format(date, i),
                "w",
                "{:<10}\t{:<10}\t{:<10}\n".format("time", "iter", "coverage"),
            )
    ...
    for i in range(num_batch * multicore):
        ...
        print(
            "[DifuzzRTL] Multicore Fuzzing Batch [{}], Thread [{}]".format(i, proc_num)
        )
